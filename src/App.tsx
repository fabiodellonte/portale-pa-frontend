...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const listRes = await api.get('/v1/segnalazioni', { headers, params: { tenant_id: tenantId, page: 1, page_size: 20, sort: 'updated_at.desc' } });
      setGovernanceItems(listRes.data?.items ?? []);
    })();
  }, [tenantId, userId, access?.can_manage_branding, access?.can_manage_roles]);

  const saveLanguage = async (e: FormEvent) => { e.preventDefault(); await api.put('/v1/me/preferences/language', { language }, { headers }); };
  const saveBranding = async (e: FormEvent) => { e.preventDefault(); await api.put(`/v1/tenants/${tenantId}/branding`, branding, { headers }); };
  const sendBug = async (e: FormEvent) => { e.preventDefault(); await api.post('/v1/bug-reports', { title: bugTitle, description: bugDescription, page_url: window.location.href }, { headers }); setBugTitle(''); setBugDescription(''); };
  const saveGlobalDoc = async (e: FormEvent) => { e.preventDefault(); await api.post('/v1/admin/docs/global', { ...globalDoc, is_published: true, sort_order: 0 }, { headers }); };
  const saveTenantDoc = async (e: FormEvent) => { e.preventDefault(); await api.post(`/v1/admin/docs/tenant/${tenantId}`, { ...tenantDoc, is_published: true, sort_order: 0 }, { headers }); };

  const firstGovernanceId = governanceItems[0]?.id;
  const updateStatus = async (e: FormEvent) => { e.preventDefault(); if (!firstGovernanceId) return; await api.post(`/v1/admin/segnalazioni/${firstGovernanceId}/status-transition`, { tenant_id: tenantId, status: governanceStatus, message: governanceStatusMsg || undefined }, { headers }); };
  const assignOperator = async (e: FormEvent) => { e.preventDefault(); if (!firstGovernanceId) return; await api.post(`/v1/admin/segnalazioni/${firstGovernanceId}/assign`, { tenant_id: tenantId, assigned_to: assignedTo, message: assignMsg || undefined }, { headers }); };
  const publishResponse = async (e: FormEvent) => { e.preventDefault(); if (!firstGovernanceId) return; await api.post(`/v1/admin/segnalazioni/${firstGovernanceId}/public-response`, { tenant_id: tenantId, message: publicResponse }, { headers }); };
  const saveFlags = async (e: FormEvent) => { e.preventDefault(); if (!firstGovernanceId) return; await api.post(`/v1/admin/segnalazioni/${firstGovernanceId}/moderation-flags`, { tenant_id: tenantId, flags: { hidden: flagHidden, abusive: flagAbusive, requires_review: flagRequiresReview, duplicate_of: flagDuplicateOf || undefined, note: moderationNote || undefined } }, { headers }); };

  return (
    <main className="shell">
      <header className="topbar"><h1>Portale PA</h1><p>Fase estesa: segnalazione bug e documentazione</p><div className="identity-grid"><label>Tenant ID<input value={tenantId} onChange={(e) => setTenantId(e.target.value)} /></label><label>User ID<input value={userId} onChange={(e) => setUserId(e.target.value)} /></label></div></header>
      <section className="page card"><h2>Impostazioni utente</h2><form onSubmit={saveLanguage}><label>Lingua<select aria-label="Lingua" value={language} onChange={(e) => setLanguage(e.target.value as 'it' | 'en')}><option value="it">Italiano</option><option value="en">English</option></select></label><button type="submit">Salva lingua</button></form></section>
      <section className="page card"><h2>Documentazione portale (consultabile da tutti)</h2><ul>{(publicDocs.global ?? []).map((d) => <li key={`g-${d.slug}`}>{d.title}</li>)}{(publicDocs.tenant ?? []).map((d) => <li key={`t-${d.slug}`}>{d.title}</li>)}</ul></section>
      <section className="page card"><h2>Segnala un bug</h2><form onSubmit={sendBug}><label>Titolo bug<input aria-label="Titolo bug" value={bugTitle} onChange={(e) => setBugTitle(e.target.value)} /></label><label>Descrizione bug<textarea aria-label="Descrizione bug" value={bugDescription} onChange={(e) => setBugDescription(e.target.value)} /></label><button type="submit">Invia bug report</button></form></section>
      {(access?.can_manage_branding || access?.can_manage_roles) && <section className="page card"><h2>Governance segnalazioni (amministrazione)</h2><form onSubmit={updateStatus}><label>Nuovo stato segnalazione<select aria-label="Nuovo stato segnalazione" value={governanceStatus} onChange={(e) => setGovernanceStatus(e.target.value)}><option value="in_attesa">in_attesa</option><option value="presa_in_carico">presa_in_carico</option><option value="in_lavorazione">in_lavorazione</option><option value="risolta">risolta</option><option value="chiusa">chiusa</option><option value="respinta">respinta</option></select></label><label>Messaggio transizione<input aria-label="Messaggio transizione" value={governanceStatusMsg} onChange={(e) => setGovernanceStatusMsg(e.target.value)} /></label><button type="submit">Aggiorna stato</button></form><form onSubmit={assignOperator}><label>ID operatore assegnazione<input aria-label="ID operatore assegnazione" value={assignedTo} onChange={(e) => setAssignedTo(e.target.value)} /></label><label>Messaggio assegnazione<input aria-label="Messaggio assegnazione" value={assignMsg} onChange={(e) => setAssignMsg(e.target.value)} /></label><button type="submit">Assegna</button></form><form onSubmit={publishResponse}><label>Messaggio pubblico segnalazione<textarea aria-label="Messaggio pubblico segnalazione" value={publicResponse} onChange={(e) => setPublicResponse(e.target.value)} /></label><button type="submit">Pubblica risposta</button></form><form onSubmit={saveFlags}><label><input aria-label="Flag nascosta" type="checkbox" checked={flagHidden} onChange={(e) => setFlagHidden(e.target.checked)} />Flag nascosta</label><label><input aria-label="Flag abusiva" type="checkbox" checked={flagAbusive} onChange={(e) => setFlagAbusive(e.target.checked)} />Flag abusiva</label><label><input aria-label="Flag richiede revisione" type="checkbox" checked={flagRequiresReview} onChange={(e) => setFlagRequiresReview(e.target.checked)} />Flag richiede revisione</label><label>Flag duplicata di<input aria-label="Flag duplicata di" value={flagDuplicateOf} onChange={(e) => setFlagDuplicateOf(e.target.value)} /></label><label>Nota moderazione<input aria-label="Nota moderazione" value={moderationNote} onChange={(e) => setModerationNote(e.target.value)} /></label><button type="submit">Salva flag</button></form></section>}
      {access?.can_manage_branding && <section className="page card"><h2>Branding comunale</h2><form onSubmit={saveBranding}><label>Colore primario<input aria-label="Colore primario" value={branding.primary_color} onChange={(e) => setBranding((b) => ({ ...b, primary_color: e.target.value }))} /></label><label>Colore secondario<input aria-label="Colore secondario" value={branding.secondary_color} onChange={(e) => setBranding((b) => ({ ...b, secondary_color: e.target.value }))} /></label><button type="submit">Salva branding</button></form></section>}
      {access?.can_manage_roles && <section className="page card"><h2>Documentazione globale amministrabile (global admin)</h2><form onSubmit={saveGlobalDoc}><label>Slug globale<input aria-label="Slug globale" value={globalDoc.slug} onChange={(e) => setGlobalDoc((d) => ({ ...d, slug: e.target.value }))} /></label><label>Titolo globale<input aria-label="Titolo globale" value={globalDoc.title} onChange={(e) => setGlobalDoc((d) => ({ ...d, title: e.target.value }))} /></label><label>Contenuto globale<textarea aria-label="Contenuto globale" value={globalDoc.content_md} onChange={(e) => setGlobalDoc((d) => ({ ...d, content_md: e.target.value }))} /></label><button type="submit">Salva doc globale</button></form></section>}
      {(access?.can_manage_branding || access?.can_manage_roles) && <section className="page card"><h2>Documentazione del comune (tenant admin)</h2><form onSubmit={saveTenantDoc}><label>Slug tenant<input aria-label="Slug tenant" value={tenantDoc.slug} onChange={(e) => setTenantDoc((d) => ({ ...d, slug: e.target.value }))} /></label><label>Titolo tenant<input aria-label="Titolo tenant" value={tenantDoc.title} onChange={(e) => setTenantDoc((d) => ({ ...d, title: e.target.value }))} /></label><label>Contenuto tenant<textarea aria-label="Contenuto tenant" value={tenantDoc.content_md} onChange={(e) => setTenantDoc((d) => ({ ...d, content_md: e.target.value }))} /></label><button type="submit">Salva doc tenant</button></form></section>}
    </main>
  );
}
